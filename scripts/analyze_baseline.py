...
def analyze(args):
    ...
    if plan["retry_policy"]["max_attempts_per_trial"] != 1:
        raise ValueError("P08 analysis requires one observed attempt per trial")
    ...
    for declared in plan["sessions"]:
        ...
        # Only declared development session and run roots; no dataset/holdout traversal.
    ...
    limitations = [
        "Descriptive evaluation of the frozen development missions in one map under a named changed-reset protocol; not a reproduction of the untouched released evaluator.",
        ...
        "No holdout pixels are loaded. Video selections are editorial static samples under the frozen execution-order rule; the complete trace remains in source evidence."
    ]
    result = {"schema_version": "vla.baseline-reset-analysis.v1", "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
              "plan_id": plan["plan_id"], "plan_sha256": sha256(args.plan), "configuration": plan["configuration"],
...
def report_markdown(result):
    ...
    lines = ["# Development baseline with independently audited reset evidence", "",
