...
The runner is implemented and covered by synthetic contract tests. A separately documented one-step published-reference run passed the P10 mechanics checks; see [the public capability summary](../../PUBLIC_STATUS.md). The default `--supervision-mode reviewed-expert` requires the approved supervision contract below. A separate explicit `--supervision-mode published-reference-mechanics` accepts five strictly validated, unchanged publisher examples for P10's one optimizer update only. It preserves unknown physical label horizon and unasserted expert approval; it does not complete P09. P12 rejects that mode. See the [published reference runbook](published_reference.md) for its source pins and preparation/validation requirements. Both modes require recorded parent model files. Test fixtures are never dataset or model evidence.
